...
model_folder = "models/stationinterp_elev_only/"
fig_folder = "figures/stationinterp_elev_only/"
# %%
...
model.model.load_state_dict(torch.load(model_folder + f"model.pt"))
# %%
...
# TODO: Use a torch dataloader and switch up the tasks for more training samples.
# %%
...
time = train_df.index.get_level_values("time").unique()[0]
# %%
...
fig.savefig(os.path.join(fig_folder, fname), bbox_inches="tight")
# %%
test_task_0context = task_loader(time, [0, "all"])
prediction = model.predict(
    test_task_0context, X_t=hires_aux_raw_ds.sel(lat=slice(55, 47.5), lon=slice(6, 15))
)["T2M"]
mean_ds_0context, std_ds_0context = prediction["mean"], prediction["std"]
...
fig, axes = gen_test_fig(
    # era5_raw_ds.sel(time=test_task['time'], lat=slice(mean_ds["lat"].min(), mean_ds["lat"].max()), lon=slice(mean_ds["lon"].min(), mean_ds["lon"].max())),
    None,
    mean_ds_0context.sel(lat=slice(latmax, latmin), lon=slice(lonmin, lonmax)),
    std_ds_0context.sel(lat=slice(latmax, latmin), lon=slice(lonmin, lonmax)),
    task=test_task_0context,
    add_colorbar=True,
    var_cbar_label="2m temperature [°C]",
    std_cbar_label="std dev [°C]",
    # std_clim=(None, 2),
...
fname = "downscale_zoom_0context.png"
fig.savefig(os.path.join(fig_folder, fname), bbox_inches="tight")
#%%
cmap = "jet"
hires_aux_raw_ds["HEIGHT"].sel(lat=slice(latmax, latmin), lon=slice(lonmin, lonmax)).plot(cmap=cmap)